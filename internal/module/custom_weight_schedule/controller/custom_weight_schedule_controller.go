...
import (
	...
	"jk-api/internal/module/custom_weight_schedule/repository"
	"jk-api/pkg/response"

	"github.com/gofiber/fiber/v2"
)

type CustomWeightScheduleController struct {
	repo repository.CustomWeightScheduleRepository
}

func NewCustomWeightScheduleController(repo repository.CustomWeightScheduleRepository) *CustomWeightScheduleController {
	return &CustomWeightScheduleController{repo: repo}
}

func (ctrl *CustomWeightScheduleController) GetAll(c *fiber.Ctx) error {
	rules, err := ctrl.repo.GetAll()
	if err != nil {
		return response.InternalServerError(c, err.Error())
	}
	return response.Success(c, "Custom weight schedules retrieved", rules)
}
...
func (ctrl *CustomWeightScheduleController) Upsert(c *fiber.Ctx) error {
	...
	rule := entity.CustomWeightSchedule{
		ID:        req.ID,
		Scope:     req.Scope,
		Enabled:   req.Enabled,
	...
	if req.ID > 0 {
		...
		return response.Success(c, "Custom weight schedule updated", rule)
	}
	if err := ctrl.repo.Create(&rule); err != nil {
		return response.InternalServerError(c, err.Error())
	}
	return response.Created(c, "Custom weight schedule created", rule)
}

func (ctrl *CustomWeightScheduleController) Delete(c *fiber.Ctx) error {
	...
	return response.Success(c, "Custom weight schedule deleted", nil)
}
